...
from BH_LymphDS.custom.image_loader import default_loader
from BH_LymphDS.custom.about_log import ColorPrinter
from BH_LymphDS.custom.about_log import logger
from BH_LymphDS.custom.common import check_file_exists, common_annotation_parser, \
...
class ListDataset(data.Dataset):
    ...
    def __init__(self, records: List[str], record_parser=None, labels_file: str = None, classes: list = None,
        ...
        for record in records:
            ...
            if check_sample_exists:
                logger.info(f"\tChecking file exists in {record}")
            else:
            ...
            if labels_file and os.path.exists(labels_file) and os.path.isfile(labels_file):
                with open(labels_file, encoding='utf8') as f:
                    ...
                    if len(self.labels) == 0:
                        raise ValueError('No matching data was found. Please check the data configuration！')
                    if not sorted(self.classes) == sorted(list(set(self.labels))):
                        if set(self.classes) - set(self.labels):
                            print(f"differences between the labels and images：{set(self.classes) - set(self.labels)} "
                                  f"or {set(self.labels) - set(self.classes)}")
...
class ListDataset4Test(data.Dataset):
    def __init__(self, data_list, transform, loader=None):
    ...
    def __getitem(self, index):
        ...
        try:
            ...
            if self.transform is not None:
                sample_ = self.transform(sample_)
            return sample_, path_
        except:
            logger.error(f"\t解析{path_}出问题，丢弃这个样本。")
            return None, None

    def __getitem__(self, index):
        sample_, path_ = self.__getitem(index)
        while sample_ is None or path_ is None:
            sample_, path_ = self.__getitem(random.randint(0, len(self.data_list)))
        return sample_, path_
...
def create_classification_dataset(dataset_name=None, **kwargs):
    """
    Create a specified dataset with given args.
