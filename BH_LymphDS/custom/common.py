...
__all__ = ['del_k_in_dict_if_exist', 'delete_dir_if_exists',
           'create_directories_if_not_exists', 'create_dir_if_not_exists', 'create_img_msk_dir',
...
def get_color_map_list(num_classes, cmap='viridis', ):
    ...
    # return color_map
    color_map = (np.array(getattr(plt.cm, cmap).colors) * 255).astype(np.uint8)
    step = color_map.shape[0] // num_classes
    return color_map[::step, :][:num_classes, :]
...
def common_annotation_parser25d(record, fname_len: int = 1, sep='\t'):
    """
    Common record parser for data samples.

    :param record: Record file has 2 or 6 columns.
    :param fname_len: 2.5D的所有的样本长度。
    :param sep: Separation of each column.
    :return: tuple of file_name, label, bbox list.
    """
    file_name_ = []
    ...
    with open(record, encoding='utf8') as f:
        for l in f.readlines():
            items = l.strip().split(sep)
            if len(items) < fname_len + 1:
                logger.error(f"文件：{record}，此行存在问题：{l}")
                raise ValueError(f"标注文件错误，样本{l.strip()}长度存在问题。")
            file_name_.append(items[:fname_len])
            label_.append(items[fname_len])
            clinic_features_.append([float(b) for b in items[fname_len + 1:]])
    return file_name_, label_, clinic_features_
